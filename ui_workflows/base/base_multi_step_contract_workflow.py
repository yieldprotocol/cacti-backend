...
class BaseMultiStepContractWorkflow(BaseContractWorkflow):
    ...
    def run(self) -> MultiStepResult:
        ...
        print(f"Multi-step contract workflow started, wf_type: {start_log_params}")
        try:
            self._setup_workflow()

            if(self._check_should_terminate_run()):
                print(f"Multi-step Workflow terminated before run, {start_log_params}")
            ...
            print(f"MULTISTEP CONTRACT WORKFLOW EXCEPTION, {start_log_params}")
            traceback.print_exc()
            return MultiStepResult(
                status='error',
                workflow_id=str(self.multistep_workflow.id),
            ...
            print(f"Multi-step contract workflow ended, {end_log_params}")
    ...
    def _run_step(self, replacement_step_type=None, extra_params=None) -> ContractStepProcessingResult:
        if replacement_step_type:
            result = self._handle_step_replace(replacement_step_type, extra_params)
        elif not self.curr_step:
            result = self._run_first_step()
        else:
            self.prev_step = self.curr_step
            result = self._run_next_step()
    ...
    def _run_first_step(self) -> ContractStepProcessingResult:
    ...
    def _handle_step_replace(self, replace_with_step_type: str, replace_extra_params: Dict) -> ContractStepProcessingResult:
        ...
        return runnable_step.function(replace_extra_params)

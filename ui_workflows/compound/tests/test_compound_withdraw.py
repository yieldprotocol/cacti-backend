...
if __name__ == "__main__":
    ...
    print(f"Confirm Withdraw of {token_to_withdraw}")

    singleStepResult: SingleStepResult = CompoundWithdrawWorkflow(wallet_chain_id, wallet_address, mock_message_id, workflow_type, worfklow_params).run()
    
    if singleStepResult.status == "success":
        tenderly_simulate_tx(wallet_address, singleStepResult.tx)
        print(f"Successfully Withdrawn {worfklow_params['amount']} {worfklow_params['token']}")
    else:

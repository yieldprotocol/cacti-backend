...
class SavingsDaiDepositContractWorkflow(BaseMultiStepContractWorkflow):
    WORKFLOW_TYPE = 'savings-dai-deposit'
    ...
    def __init__(self, wallet_chain_id: int, wallet_address: str, chat_message_id: str, workflow_params: Dict, multistep_workflow: Optional[MultiStepWorkflow] = None, curr_step_client_payload: Optional[WorkflowStepClientPayload] = None) -> None:
        ...
        # The only token that can be deposited is DAI, you have to handle approval before final confirmation
        initiate_erc20_approval_step = RunnableStep("initiate_ERC20_approval", WorkflowStepUserActionType.tx, f"Approve deposit of {self.amount} {self.token} on SavingsDAI", self.initiate_erc20_approval_step)
        confirm_erc4626_deposit_step = RunnableStep("confirm_ERC4626_deposit", WorkflowStepUserActionType.tx, f"Confirm deposit of {self.amount} {self.token} on SavingsDAI", self.confirm_erc4626_deposit_step)
        steps = [initiate_erc20_approval_step, confirm_erc4626_deposit_step]

        final_step_type = "confirm_erc4626_deposit"
    ...
    def confirm_ERC20_supply_step(self, extra_params=None) -> ContractStepProcessingResult:
        ...
        referral_code = 0
        encoded_data = get_savings_dai_address_contract().encodeABI(fn_name='deposit', args=[amount, self.wallet_address])
        tx = {
